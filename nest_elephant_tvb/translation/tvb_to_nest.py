...
if __name__ == "__main__":
    ...
    id_first_spike_generator = int(sys.argv[2])
    ...
    logger_master = create_logger(path_config, 'tvb_to_nest_master'+str(id_first_spike_generator), log_level)
    logger_send = create_logger(path_config, 'tvb_to_nest_send'+str(id_first_spike_generator), log_level)
    logger_receive = create_logger(path_config, 'tvb_to_nest_receive'+str(id_first_spike_generator), log_level)
    ############
    
    ############ Step 3: RichEndPoint -- open MPI connections
    ### TODO: make this a proper interface
    path_to_files_receive = path_config + TVB_config
    logger_master.info(f"Path to files receive: {path_to_files_receive}")
    path_to_files_send = os.path.join(path_config, str(id_first_spike_generator) + ".txt")
    logger_master.info(f"Path to files send: {path_to_files_send}")
    ...
    for i in range(nb_spike_generator):
        ...
        logger_master.info(f"Writing port to: {path_to_files_send}")
        ...
        logger_master.info(f"Touching unlock file: {path_to_files_send_unlock}")
    ...
    ttn.init(path_config, nb_spike_generator, id_first_spike_generator, param,
